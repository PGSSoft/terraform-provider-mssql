...
func WrapDataSource[T any](d DataSource[T]) datasource.DataSourceWithConfigure {
...
func (d dataSourceWrapper[T]) Metadata(ctx context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = fmt.Sprintf("%s_%s", req.ProviderTypeName, d.d.GetName())
}

func (d dataSourceWrapper[T]) Configure(ctx context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
	...
	d.d.Configure(utils.WithDiagnostics(ctx, &resp.Diagnostics), ConfigureRequest{Conn: db})
}
