...
func NewContext(providerFactory func(connection sql2.Connection) provider.Provider) *TestContext {
	testCtx := TestContext{
...
func (t *TestContext) CreateDB(test *testing.T, dbName string) int {
	dbId, err := t.tryCreateDB(dbName, false)
	require.NoError(test, err, "Failed to create DB %q", dbName)
	return dbId
}

func (t *TestContext) ExecDB(test *testing.T, dbName string, statFmt string, args ...any) {
	err := t.execDB(dbName, statFmt, args...)
	require.NoError(test, err, "Failed to execute SQL")
}

func (t *TestContext) ExecDefaultDB(test *testing.T, statFmt string, args ...any) {
	t.ExecDB(test, DefaultDbName, statFmt, args...)
}

func (t *TestContext) ExecMasterDB(test *testing.T, statFmt string, args ...any) {
	t.ExecDB(test, "master", statFmt, args...)
}
